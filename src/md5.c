...
static const uint32_t g_md5_r[] = {7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17,
...
void md5_turn1(int i, int *f, int *g, int abcd[4])
{
	*f = (abcd[1] & abcd[2]) | ((~abcd[1]) & abcd[3]);
	*g = i;
}

void md5_turn2(int i, int *f, int *g, int abcd[4])
{
	*f = (abcd[3] & abcd[1]) | ((~abcd[3]) & abcd[2]);
	*g = (5 * i + 1) % 16;
}

void md5_turn3(int i, int *f, int *g, int abcd[4])
{
	*f = abcd[1] ^ abcd[2] ^ abcd[3];
	*g = (3 * i + 5) % 16;
}

void md5_turn4(int i, int *f, int *g, int abcd[4])
{
...
void md5_digest_turn(int i, int *w, int abcd[4])
{
	...
	if (i < 16)
		md5_turn1(i, &f, &g, abcd);
	else if (i < 32)
		md5_turn2(i, &f, &g, abcd);
	else if (i < 48)
		md5_turn3(i, &f, &g, abcd);
	else
		md5_turn4(i, &f, &g, abcd);
...
char get_hex_char(unsigned char quartet)
{
    quartet = quartet & 0xf;
    if (quartet < 10)
        return (quartet + '0');
...
char *md5tostr(unsigned int hashbuff[4])
{
	char *out;
	int i;
    uint8_t *p;
	...
	while (i < 4)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			out[i * 8 + j * 2] = get_hex_char((p[j] >> 4));
            out[i * 8 + j * 2 + 1] = get_hex_char(p[j]);
		}
		i++;
	}
	return (out);
